...
if ($gps_dat->{Activities}) {
  foreach $point (@{$gps_dat->{Activities}->{Activity}->{Lap}->{Track}->{Trackpoint}}) {
    $time = format_time($point->{Time};
      if ($start_flag) {
        $start_time = $time;
        $start_flag = 0;
        $prev_lat = $point->{Position}->{LatitudeDegrees};
        $prev_long = $point->{Position}->{LongitudeDegrees};
        $prev_elev = $point->{AltitudeMeters};
      }
    ...
    @point_spread = ($point->{Position}->{LatitudeDegrees}, $prev_lat, $point->{Position}->{LongitudeDegrees}, $prev_long, $point->{AltitudeMeters}, $prev_elev);
    $curr_dist = calc_distance(@point_spread);
    $totes_dist += ($curr_dist * 3.28084);

    # now (b) pace in min / mile
    if ($totes_dist != 0) {
      $pace = 88 * ($agg_time / $totes_dist);
      $pace_min = int(88 * ($agg_time / $totes_dist));
    ...
    printf '{"lat":"%.6f","lng":"%.6f","elev":"%.4f","elapsed":"%s","distance":"%.6f","pace":"%.4f"},', $point->{lat}, $point->{lon}, $point->{ele} * 3.28084, $agg_time, $totes_dist, $pace;
    printf "\n";

  }
}

foreach $point (@{$gps_dat->{trk}->{trkseg}->{trkpt}}) {
  $time = format_time($point->{time});
  if ($start_flag) {
    $start_time = $time;
    $start_flag = 0;
